...
def RUN(path, modelname):
    ...
    classifier = SimpleClassifier(output_dim =len(label2id))
    ...
    f=open('Morph_results/first_sub/'+path+'_'+modelname.replace("/", "_")+"_morphology_freeze.pkl", "wb")
    pickle.dump(best_accs, f)
    f.close()
    torch.save(best_model, 'Morph_results/first_sub/'+path+'_'+modelname.replace("/", "_")+"freeze.pth")
    gc.collect()
...
if __name__ == "__main__":
    ...
    plt.savefig('Morph_Freeze_F1.png')
    ...
    plt.savefig('Morph_Freeze_Time.png')
